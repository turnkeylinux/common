...
function addAD () {
    ...
    if (query) {
        chrome.extension.sendRequest({'action':'getAD','query':query,'url': window.location.href},function (response) {
            console.log(response);
            var lists = response.data,
                length = lists.length > 3 ? 3 :lists.length;

            if (document.getElementById('sa-admanage-box')) {
                $('#sa-admanage-box').remove();
            }

            var adBox = document.createElement('div');
            adBox.id = "sa-admanage-box";
            adBox.innerHTML = '<h2><span>Ads not from this site</span></h2>';

            var ul = document.createElement('ol');

            for (var i = 0;i < length;i++) {
                var title = myReplace(query, lists[i].title['#cdata']);
                var des = myReplace(query, lists[i].descr['#cdata']);
                var host = myReplace(query, lists[i].host['#cdata']);
                var url = lists[i].url['#cdata'];


                var li = '<li class="Admanage-li"><h3><a target="_blank" href="'+ url +'">'+ title +'</a></h3><div><cite>'+host+'</cite></div><span>'+des+'</span></li>';

                ul.innerHTML += li;
            }

            adBox.appendChild(ul);
            $('#center_col').prepend(adBox);
        });
    }
